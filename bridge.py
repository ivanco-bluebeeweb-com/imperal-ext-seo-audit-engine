...
def max_run_id(path: str) -> int:
    """Наибольший id прогона в файле базы. 0 — прогонов нет."""
...
async def upload_run_safely(ctx, local_path: str, run_id: int,
                             base_max_run_id: int) -> int:
    """Залить локальную базу, не стирая чужой прогон, записанный параллельно.

    ПОЧЕМУ ЭТО НУЖНО. `download_db`/`upload_db` работают со ВСЕЙ базой
    портфеля: один инструмент скачивает снимок, работает минуты, заливает
    обратно целиком. Если второй вызов (повтор из-за таймаута, параллельный
    ручной аудит, ночной прогон по расписанию) стартовал почти одновременно,
    оба видят один и тот же снимок и последняя заливка молча стирает прогон,
    записанный первым — без единой ошибки для пользователя. Обнаружено на
    живом портфеле: `audit_sites` дважды вернул `run_id: 9, finished: true`,
    но ни разу это не читалось обратно ни `get_report`, ни `list_runs`.

    `base_max_run_id` — наибольший id прогона в БАЗЕ, СКАЧАННОЙ В НАЧАЛЕ этого
    вызова (до того как локально появился новый `run_id`). Перед заливкой мы
    смотрим на АКТУАЛЬНОЕ состояние хранилища:

    - если там всё ещё не больше `base_max_run_id` прогонов — никто не залил
      раньше нас, можно писать локальную базу как есть (обычный путь);
    - если там уже больше — кто-то залил свежее хранилище, пока мы работали;
      тогда наш новый прогон переносится (`Store.copy_run`) в ЭТУ свежую копию
      вместо того, чтобы её перезаписать, и заливается уже слитая версия.

    Это не полная сериализация (окно между повторным скачиванием и заливкой
    остаётся, хоть и намного короче исходного), а конкретное устранение
    единственного реально наблюдённого сценария потери данных. Возвращает
    итоговый id прогона (может отличаться от `run_id`, если пришлось слить —
    автоинкремент целевой базы свой).
    """
    remote_path = await download_db(ctx)
    remote_max = max_run_id(remote_path) if remote_path else 0

    if not remote_path or remote_max <= base_max_run_id:
        # Никто не обогнал нас — заливаем локальную базу как есть.
        await upload_db(ctx, local_path)
        return run_id

    # Кто-то залил более новый прогон, пока мы работали. Не перезаписываем —
    # переносим наш прогон в актуальную копию и заливаем её.
    local_store = open_store(local_path)
    remote_store = open_store(remote_path)
    try:
        merged_run_id = local_store.copy_run(run_id, remote_store)
    finally:
        local_store.close()
        remote_store.close()
    await upload_db(ctx, remote_path)
