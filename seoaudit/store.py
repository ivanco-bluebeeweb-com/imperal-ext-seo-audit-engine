...
class Store:
    ...
    def close(self) -> None:
        ...
        ПОЧЕМУ СНАЧАЛА CHECKPOINT. При многосайтовом прогоне (`site_workers`
        > 1) каждый воркер-поток открывает СВОЁ соединение (см. класс-докстринг),
        а sqlite3 запрещает закрывать соединение не из того потока, где оно
        создано — `close()` из чужого потока бросает `ProgrammingError`, которую
        цикл ниже намеренно проглатывает (один сайт не должен ронять прогон).
        Но WAL-режим держит свежие коммиты в `-wal`-файле, пока кто-то не
        сделает checkpoint; если соединение воркера так и не закрылось, его
        данные могут остаться ТОЛЬКО там. `upload_db` заливает raw-байты
        одного файла `path` — без явного checkpoint это ровно то же самое,
        что закрыть глаза на половину прогона: находки второго сайта из
        параллельного `site_workers`-прогона тихо не долетают до хранилища
        (обнаружено на живом двухсайтовом прогоне: audit_sites вернул
        finished=true и валидный run_id, но ни list_runs, ни get_report
        этот прогон после не находили).

        Checkpoint запускаем с СОВЕРШЕННО НОВОГО соединения, а не с одного из
        `self._connections` — проверено, что это работает независимо от того,
        из какого потока вызван `close()`, и не зависит от того, сколько
        воркер-соединений успешно закроются ниже.
        """
        try:
            ckpt_con = sqlite3.connect(self.path, timeout=30.0)
            try:
                ckpt_con.execute("PRAGMA wal_checkpoint(TRUNCATE)")
            finally:
                ckpt_con.close()
        except Exception:
            pass  # чекпойнт — лучшее усилие; заливка всё равно должна пройти
