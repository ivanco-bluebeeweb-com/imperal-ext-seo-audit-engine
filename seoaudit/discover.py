...
def with_scheme(site: str) -> str:
    """Дописать https://, если домен назван без схемы.

    ЗАЧЕМ ЭТО НУЖНО. `normalize_url` опирается на `urlsplit`, а тот в строке
    `climtec.md` видит ПУТЬ, а не хост. Схема тогда приклеивается к пути, и
    получается `https:climtec.md` — адрес без хоста, который не открывается
    ничем: прогон честно отрабатывает и возвращает одну страницу с ошибкой
    «no host given», то есть выглядит как «сайт лежит», а не как «мы его не
    туда попросили».

    Домысливать схему можно ТОЛЬКО там, где домен ввёл человек. Для ссылок,
    найденных на страницах, голая строка без схемы — это настоящий
    относительный путь, и та же догадка была бы ошибкой. Поэтому функция
    отдельная и вызывается на границе ввода, а не внутри `normalize_url`.
    """
    s = (site or "").strip()
    if not s:
        return ""
